...
export const getCreateEmployeeSchema = (t: (key: string, module?: string) => string) => {
  ...
  return z.object({
    ...
    grandfather_name: z.string().min(1,  t('employee_form.validation.grandfather_name_required', 'hr') || 'اسم الجد مطلوب' ),
    ...
    assigned_job: z.string().min(1,  t('employee_form.validation.assigned_job_required', 'hr') || 'العمل المكلف به مطلوب' ),
    ...
    number_of_children: z.number().min(0, t('employee_form.validation.number_of_children_invalid', 'hr') || 'عدد الأولاد يجب أن يكون صفراً أو موجباً'),

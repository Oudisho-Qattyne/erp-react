...
export const getCreateEmployeeSchema = (t: (key: string, module?: string) => string) => {
  const EducationEntrySchema = z.object({
    ...
    degree_name: z.string().min(1, t('employee_form.validation.degree_name_required', 'hr') || 'اسم الشهادة مطلوب').nullable().optional(),
    ...
    graduation_year: z.string().regex(/^\d{4}$/, t('employee_form.validation.graduation_year_invalid', 'hr') || 'سنة التخرج غير صالحة (يجب أن تكون 4 أرقام)').nullable().optional(),
  ...
  return z.object({
    ...
    place_birth: z.string().min(1, t('employee_form.validation.place_birth_required', 'hr') || 'مكان الميلاد مطلوب').nullable().optional(),
    ...
    spouse_workplace: z.string().nullable().optional(),
    ...
    sham_cash_account: z.string().nullable().optional(),
    ...
    residential_area_details: z.string().nullable().optional(),
    civil_registry_record: z.string().nullable().optional(),

...
export function ServiceConditionsPage() {
  const { t } = useLanguage();
  const { entities: items, getAll, create, update, remove, loadingMap, errorMap } = useEntityCrud<ServiceCondition>('/investments/service-conditions', '/investments/service-conditions');
  const entityName = t('service_conditions.title', 'investments') || 'Service Condition';
  const [searchQuery, setSearchQuery] = useState('');
  const [isCreateOpen, setIsCreateOpen] = useState(false);
  const [editItem, setEditItem] = useState<ServiceCondition | null>(null);
  const [confirmDelete, setConfirmDelete] = useState<ServiceCondition | null>(null);
  const [confirmSetDefault, setConfirmSetDefault] = useState<ServiceCondition | null>(null);
  const [auditItem, setAuditItem] = useState<ServiceCondition | null>(null);
  ...
  const columns = [
    {
      key: 'name',
      label: t('service_conditions.name', 'investments') || 'Condition Name',
      width: 250,
      render: (row: ServiceCondition) => row.name
    },
    {
      key: 'is_active',
    ...
    },
    {
      ...
      width: 200,
      render: (row: ServiceCondition) => (
        <div className="flex items-center justify-center gap-1" onClick={e => e.stopPropagation()}>
          {!row.is_default && (
            <Button variant="ghost" size="sm" onClick={() => setConfirmSetDefault(row)}
              title={t('common.set_default', 'shared') || 'Set as default'} requiredPermission="investments.service-conditions.update">
              <Star size={16} />
            </Button>
          )}
          <Button variant="ghost" size="sm" onClick={() => setEditItem(row)}
            title={t('common.edit', 'shared') || 'Edit'} requiredPermission="investments.service-conditions.update">
            <Pencil size={16} />
          </Button>
          <Button variant="ghost" size="sm" onClick={() => setConfirmDelete(row)}
            title={t('common.delete', 'shared') || 'Delete'} requiredPermission="investments.service-conditions.delete">
            <Trash2 size={16} className="text-danger" />
          </Button>
          <Button variant="ghost" size="sm" onClick={() => setAuditItem(row)}
            title={t('service_conditions.edit_log', 'investments') || 'Edit Log'} requiredPermission="shared.audit-logs.view">
            <History size={16} />
          </Button>
        </div>
      )
  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex flex-col gap-2">
        <h1 className="text-2xl font-bold">{t('service_conditions.title', 'investments')}</h1>
        <div className="w-full flex gap-2">
          <Input type="text" value={searchQuery} onChange={setSearchQuery}
            placeholder={t('common.search', 'shared') || 'Search...'}
            baseClasses={inputBaseClasses} className="w-60" />
            <Button onClick={() => setIsCreateOpen(true)} requiredPermission="investments.service-conditions.create">{t('service_conditions.add', 'investments')}</Button>
        </div>
      </div>

      <Dialog isOpen={isCreateOpen} onClose={() => setIsCreateOpen(false)} title={t('service_conditions.add', 'investments')}>
        <GenericCreateForm
          fields={[
      ...
      toast.error(err?.message || t('service_conditions.create_error', 'investments').replace('{name}', entityName)); throw err;
          ...
          submitLabel={t('service_conditions.add', 'investments')}
        />
      </Dialog>
      ...
      {!errorMap['getAll'] && (
        <DataTable columns={columns} data={filtered} rowKey="id" loading={loadingMap['getAll']}
          emptyMessage={t('service_conditions.no_records', 'investments')} />
      )}
      ...
      <AuditLog
        ...
        model="service_condition"
        modelId={auditItem?.id}
        module="investments"
        labels={{
          title: t('service_conditions.edit_log', 'investments') || 'Edit Log',
          event: t('service_conditions.event', 'investments') || 'Event',
          created_at: t('service_conditions.created_at', 'investments') || 'Created At',
          changed_by: t('service_conditions.changed_by', 'investments') || 'Changed By',
          changes: t('service_conditions.changes', 'investments') || 'Changes',
          field: t('service_conditions.field', 'investments') || 'Field',
          old_value: t('service_conditions.old_value', 'investments') || 'Old Value',
          new_value: t('service_conditions.new_value', 'investments') || 'New Value',
          no_records: t('service_conditions.no_edit_log', 'investments') || 'No edit logs found',
          subject_id: t('service_conditions.subject_id', 'investments') || 'Service Condition ID',
        }}
        translateField={(key) => t(`service_conditions.${key}`, 'investments') || key}

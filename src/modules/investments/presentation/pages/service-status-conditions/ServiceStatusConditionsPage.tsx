...
export function ServiceStatusConditionsPage() {
  const { t } = useLanguage();
  const { entities: items, getAll, create, update, remove, loadingMap, errorMap } = useEntityCrud<ServiceStatusCondition>('/investments/service-status-conditions', '/investments/service-status-conditions');
  const entityName = t('service_status_conditions.title', 'investments') || 'Service Status Condition';
  const [searchQuery, setSearchQuery] = useState('');
  const [isCreateOpen, setIsCreateOpen] = useState(false);
  const [editItem, setEditItem] = useState<ServiceStatusCondition | null>(null);
  const [confirmDelete, setConfirmDelete] = useState<ServiceStatusCondition | null>(null);
  const [confirmSetDefault, setConfirmSetDefault] = useState<ServiceStatusCondition | null>(null);
  const [auditItem, setAuditItem] = useState<ServiceStatusCondition | null>(null);
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      ...
      toast.success(t('service_status_conditions.deleted', 'investments').replace('{name}', entityName));
    } catch (err: any) {
      toast.error(err?.message || t('service_status_conditions.delete_error', 'investments').replace('{name}', entityName));
    }
  ...
  const columns = [
    {
      key: 'name',
      label: t('service_status_conditions.name', 'investments') || 'Condition Name',
      width: 250,
      render: (row: ServiceStatusCondition) => row.name
    },
    {
      key: 'is_active',
      label: t('common.is_active', 'shared') || 'Is Active?',
      width: 120,
      render: (row: ServiceStatusCondition) => row.is_active
    ...
    },
    {
      ...
      width: 120,
      render: (row: ServiceStatusCondition) => row.is_default
    ...
    },
    {
      ...
      width: 200,
      render: (row: ServiceStatusCondition) => (
        <div className="flex items-center justify-center gap-1" onClick={e => e.stopPropagation()}>
          {!row.is_default && (
            <Button variant="ghost" size="sm" onClick={() => setConfirmSetDefault(row)}
              title={t('common.set_default', 'shared') || 'Set as default'} requiredPermission="investments.service-status-conditions.update">
              <Star size={16} />
            </Button>
          )}
          <Button variant="ghost" size="sm" onClick={() => setEditItem(row)}
            title={t('common.edit', 'shared') || 'Edit'} requiredPermission="investments.service-status-conditions.update">
            <Pencil size={16} />
          </Button>
          <Button variant="ghost" size="sm" onClick={() => setConfirmDelete(row)}
            title={t('common.delete', 'shared') || 'Delete'} requiredPermission="investments.service-status-conditions.delete">
            <Trash2 size={16} className="text-danger" />
          </Button>
          <Button variant="ghost" size="sm" onClick={() => setAuditItem(row)}
            title={t('service_status_conditions.edit_log', 'investments') || 'Edit Log'} requiredPermission="shared.audit-logs.view">
            <History size={16} />
          </Button>
        </div>
      )
  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex flex-col gap-2">
        <h1 className="text-2xl font-bold">{t('service_status_conditions.title', 'investments')}</h1>
      ...
      toast.error(err?.message || t('service_status_conditions.create_error', 'investments').replace('{name}', entityName)); throw err;
    }
          }}
          onSuccess={() => { toast.success(t('service_status_conditions.created', 'investments').replace('{name}', entityName)); getAll(); setIsCreateOpen(false); }}
          onCancel={() => setIsCreateOpen(false)}
          submitLabel={t('service_status_conditions.add', 'investments')}
        />
      </Dialog>
      ...
      {!errorMap['getAll'] && (
        <DataTable columns={columns} data={filtered} rowKey="id" loading={loadingMap['getAll']}
          emptyMessage={t('service_status_conditions.no_records', 'investments')} />
      )}
      ...
      <AuditLog
        ...
        model="service_status_condition"
        modelId={auditItem?.id}
        module="investments"
        labels={{
          title: t('service_status_conditions.edit_log', 'investments') || 'Edit Log',
          event: t('service_status_conditions.event', 'investments') || 'Event',
          created_at: t('service_status_conditions.created_at', 'investments') || 'Created At',
          changed_by: t('service_status_conditions.changed_by', 'investments') || 'Changed By',
          changes: t('service_status_conditions.changes', 'investments') || 'Changes',
          field: t('service_status_conditions.field', 'investments') || 'Field',
          old_value: t('service_status_conditions.old_value', 'investments') || 'Old Value',
          new_value: t('service_status_conditions.new_value', 'investments') || 'New Value',
          no_records: t('service_status_conditions.no_edit_log', 'investments') || 'No edit logs found',
          subject_id: t('service_status_conditions.subject_id', 'investments') || 'Service Status Condition ID',
        }}
        translateField={(key) => t(`service_status_conditions.${key}`, 'investments') || key}

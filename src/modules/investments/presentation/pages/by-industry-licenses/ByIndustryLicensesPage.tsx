...
export function ByIndustryLicensesPage() {
  const { t } = useLanguage();
  const { entities: items, create, update, remove, loadingMap, errorMap, list } = useEntityCrud<ByIndustryLicense>(
    '/investments/by-industry-licenses',
    '/investments/by-industry-licenses',
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      ...
      toast.success(t('by_industry_licenses.deleted', 'investments').replace('{name}', entityName));
    } catch (err: any) {
  ...
  const columns = [
    ...
    {
      key: 'created_at',
      label: t('by_industry_licenses.created_at', 'investments') || 'Created At',
      width: 180,
      sortable: true,
      render: (row: ByIndustryLicense) => row.created_at || '—',
    },
    {
      key: 'actions',
      label: t('common.actions', 'shared') || 'Actions',
      width: 200,
      render: (row: ByIndustryLicense) => (
        <div className="flex items-center justify-center gap-1" onClick={e => e.stopPropagation()}>
          {!row.is_default && (
            <Button variant="ghost" size="sm" onClick={() => setConfirmSetDefault(row)}
              title={t('common.set_default', 'shared') || 'Set as default'} requiredPermission="investments.by-industry-licenses.update">
              <Star size={16} />
            </Button>
          )}
          <Button variant="ghost" size="sm" onClick={() => setEditItem(row)}
            title={t('common.edit', 'shared') || 'Edit'} requiredPermission="investments.by-industry-licenses.update">
            <Pencil size={16} />
          </Button>
          <Button variant="ghost" size="sm" onClick={() => setConfirmDelete(row)}
            title={t('common.delete', 'shared') || 'Delete'} requiredPermission="investments.by-industry-licenses.delete">
            <Trash2 size={16} className="text-danger" />
          </Button>
          <Button variant="ghost" size="sm" onClick={() => setAuditItem(row)}
            title={t('by_industry_licenses.edit_log', 'investments') || 'Edit Log'} requiredPermission="shared.audit-logs.view">
            <History size={16} />
          </Button>
        </div>
      )
  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex flex-col gap-2">
        <h1 className="text-2xl font-bold">{t('by_industry_licenses.title', 'investments')}</h1>
      ...
      handleApiError(err, { module: "investments" }); throw err;
          ...
          onSuccess={() => { toast.success(t('by_industry_licenses.created', 'investments').replace('{name}', entityName)); list.refresh(); setIsCreateOpen(false); }}
      ...
      <AuditLog
        ...
        model="by_industry_license"
        modelId={auditItem?.id}
        module="investments"
        labels={{
          title: t('by_industry_licenses.edit_log', 'investments') || 'Edit Log',
          event: t('by_industry_licenses.event', 'investments') || 'Event',
          created_at: t('by_industry_licenses.created_at', 'investments') || 'Created At',
          changed_by: t('by_industry_licenses.changed_by', 'investments') || 'Changed By',
          changes: t('by_industry_licenses.changes', 'investments') || 'Changes',
          field: t('by_industry_licenses.field', 'investments') || 'Field',
          old_value: t('by_industry_licenses.old_value', 'investments') || 'Old Value',
          new_value: t('by_industry_licenses.new_value', 'investments') || 'New Value',
          no_records: t('by_industry_licenses.no_edit_log', 'investments') || 'No edit logs found',
          subject_id: t('by_industry_licenses.subject_id', 'investments') || 'By Industry License ID',
        }}
        translateField={(key) => t(`by_industry_licenses.${key}`, 'investments') || key}

...
export function PlotClassificationsPage() {
  const { t } = useLanguage();
  const { entities: classifications, create, update, remove, loadingMap, errorMap, list } = useEntityCrud<PlotClassification>(
    '/investments/plot-classifications',
    '/investments/plot-classifications',
    { listState: true, defaultSortColumn: 'name', paginate: false, debounceMs: 300 }
  );
  const entityName = t('plot_classifications.title', 'investments') || 'Plot Classification';
  ...
  const columns = [
    ...
    {
      key: 'created_at',
      label: t('plot_classifications.created_at', 'investments') || 'Created At',
      width: 180,
      sortable: true,
      render: (row: PlotClassification) => row.created_at || '—',
    },
    {
      key: 'actions', 
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      handleApiError(err, { module: "investments" }); throw err; 
          ...
          onSuccess={() => { toast.success(t('plot_classifications.created', 'investments').replace('{name}', entityName)); list.refresh(); setIsCreateOpen(false); }}

...
export function PartnershipTypesPage() {
  ...
  const filtered = items.filter((c) => getLocalizedName(c.name).toLowerCase().includes(searchQuery.toLowerCase()));
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      handleApiError(err, { module: "investments" }); throw err;
          ...
          onSuccess={() => { toast.success(t('partnership_types.created', 'investments').replace('{name}', entityName)); getAll(); setIsCreateOpen(false); }}

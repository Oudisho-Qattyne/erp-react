...
export function LicensingStatusesPage() {
  const { t } = useLanguage();
  const { entities: items, create, update, remove, loadingMap, errorMap, list } = useEntityCrud<LicensingStatus>(
    '/investments/license-statuses',
    '/investments/license-statuses',
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      handleApiError(err, { module: "investments" }); throw err;
          ...
          onSuccess={() => { toast.success(t('licensing_statuses.created', 'investments').replace('{name}', entityName)); list.refresh(); setIsCreateOpen(false); }}

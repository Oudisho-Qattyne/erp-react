...
export function RentContractIndustriesPage() {
  const { t } = useLanguage();
  const { entities: items, create, update, remove, loadingMap, errorMap, list } = useEntityCrud<RentContractIndustry>(
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      handleApiError(err, { module: "investments" }); throw err;
          ...
          onSuccess={() => { toast.success(t('rent_contract_industries.created', 'investments').replace('{name}', entityName)); list.refresh(); setIsCreateOpen(false); }}

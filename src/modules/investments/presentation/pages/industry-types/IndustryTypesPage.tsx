...
export function IndustryTypesPage() {
  const { t } = useLanguage();
  const { entities: items, getAll, create, update, remove, loadingMap, errorMap } = useEntityCrud<IndustryType>('/investments/industry-types', '/investments/industry-types');
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      ...
      toast.error(err?.message || t('industry_types.delete_error', 'investments').replace('{name}', entityName));
    }
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      toast.error(err?.message || t('industry_types.create_error', 'investments').replace('{name}', entityName)); throw err;
    }
          }}
